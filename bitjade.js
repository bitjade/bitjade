...
fetchPoolData()
    .then(poolData => {
        ...
        coinData.forEach(coin => {
            ...
            symbolText.classList.add('token-item');
            symbolText.textContent = coin.type; // Assuming coin object has a `type` property

            const algoText = document.createElement('div');
            algoText.classList.add('token-item');
            algoText.textContent = coin.algorithm; // Assuming coin object has an `algorithm` property
            ...
            coinGrid.appendChild(symbolText);
            coinGrid.appendChild(algoText);
